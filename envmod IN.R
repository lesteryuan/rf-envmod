...
#    new.data$pctcrop2019ws <- seq(min(df1$pctcrop2019ws),
#                                  max(df1$pctcrop2019ws), length = 40)

    new.data$wetindexws <- seq(min(df1$wetindexws),
                               max(df1$wetindexws), length = 40)
    new.data2 <- df1
    new.data2$pctcrop2019ws <- 0

    mod <- ranger(data = df1[, c(var, varlist)],
                  dependent.variable.name = var,
                  num.trees = 5000, importance = "permutation")
    print(mod)
    plot(mod$predictions, df1[, var])
    ...
    plot(new.data$wetindexws, predout$predictions)
    stop()
    predout <- predict(mod, new.data2)

    dev.new()
    plot(mod$predictions, predout$predictions - mod$predictions, col = "grey")
    abline(h = 0)
